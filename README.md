...
- `src/app/base.rs` contains Base/Bitable command execution; `src/app/base/`
  contains media, record, reference, schema, field/list, search, view,
...
- `src/app/task.rs` contains Task command execution; `src/app/task/` contains
  tasklist execution, Task body/query helpers, input/time/reminder
  normalization helpers, and tasklist/comment/member collaboration builders
  plus section/custom-field and reminder/dependency relation builders.
...
- `src/app/wiki.rs` contains Wiki/knowledge-space command execution and route
  checks; `src/app/wiki/` contains typed Wiki request body builders plus
  route-check/write-probe helpers.
