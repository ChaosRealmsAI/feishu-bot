...
- `src/app/help.rs` exposes AI help text; `src/app/help/` splits long
  ...
  static scope registry in `src/app/scopes/`; `src/app/output.rs` re-exports
  output helpers, while `src/app/output/` contains generic response summaries,
  document block/code output, and Feishu block/code label maps.
